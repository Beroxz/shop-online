...
$sqlProductsType = "SELECT * FROM products_type WHERE pt_id = '" . $_GET["pt_id"] . "'";
$queryProductsType = mysqli_query($conn, $sqlProductsType);
$fetchProductsType = mysqli_fetch_assoc($queryProductsType);
...
<body class="d-flex flex-column min-vh-100">
                ...
                <div class="col mb-5">
                    <div class="card h-100 shadow rounded-4">
                        <div class="card-body px-5 py-5">
                            <h2 class="text-center fw-bold">แก้ไขประเภทสินค้า</h2>
                            <form class="row g-3 mt-3 needs-validation" novalidate method="post" action="" enctype="multipart/form-data">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control rounded-3" id="productType" name="productType" value="<?php echo $fetchProductsType["pt_name"] ?>" required>
                                </div>
    ...
    if (!empty($_POST["submit"])) {
        ...
        if ($num > 0) {
            ...
            $sqlUpProductsType = "UPDATE `products_type` SET `pt_name`='" . $productType . "' WHERE `pt_id` = '".$_GET["pt_id"]."'";
            $resUpProductsType = mysqli_query($conn, $sqlUpProductsType);
            if ($resUpProductsType) {
    ?>
                <script type="text/javascript">
                    swal.fire({
                        title: "Successfully",
                        html: "แก้ไขประเภทสินค้าสำเร็จแล้ว",
                        icon: "success",
                        timer: 2000,
                        allowOutsideClick: false
                    }).then(function() {
            ...
            ?>
                <script type="text/javascript">
                    swal.fire({
                        ...
                        window.location.href = 'edit-product-type';
                    })
                </script>
    <?php
            }
